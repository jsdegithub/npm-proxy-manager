...
const { NPRC, NPMRC, PROXY, HTTPSPROXY, REGISTRIES } = require('./constants');
...
function geneDashLine(message = '', length = 0) {
  const finalMessage = new Array(Math.max(2, length - message.length + 5)).join(
    '-'
  );
...
async function getCurrentHTTPSProxy() {
  const npmrc = await readFile(NPMRC);
  return npmrc[HTTPSPROXY];
}
